...
actor DictionaryLookupRuntime {
    ...
    func lookup(_ text: String, maxResults: Int, scanLength: Int) async throws -> DictionaryLookupResult {
        ...
        return DictionaryLookupResult(
            query: trimmed,
            entries: LookupRanking.rank(
                rawResults.map(Self.makeEntry),
                dictionaryOrder: enabledTermDictionaryOrder()
            ),
            isPlaceholder: false,
    ...
    private func enabledTermDictionaryOrder() -> [String: Int] {
        var order: [String: Int] = [:]
        for d in termDictionaries where d.info.isEnabled {
            if order[d.info.title] == nil { order[d.info.title] = order.count }
        }
        return order
    }
    ...
    private func applyOrder(_ ids: [String], to dictionaries: [ManagedDictionary]) -> [ManagedDictionary] {
        var byID = [String: ManagedDictionary](minimumCapacity: dictionaries.count)
